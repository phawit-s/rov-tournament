...
export type DonateConfig = {
  ...
  /**
   * ลิงก์ตัวกลางตรวจสลิป (Cloudflare Worker ที่ deploy เอง)
   * ว่างไว้ = ตรวจแค่ว่าสลิปซ้ำไหม ซึ่งทำงานได้โดยไม่ต้องมีเซิร์ฟเวอร์
   * ใส่แล้ว = ยืนยันกับธนาคารว่าเงินเข้าจริงและยอดตรง
   * คีย์ของผู้ให้บริการอยู่ใน Worker ไม่ได้อยู่ในหน้าเว็บ
   */
  verifyEndpoint?: string;
  /** อนุมัติอัตโนมัติเมื่อยืนยันผ่านและยอดตรง */
  autoApprove?: boolean;
};
...
export type Donation = {
  ...
  /* ---- ผลการตรวจสลิป ---- */
  /** SHA-256 ของ QR บนสลิป ใช้กันส่งสลิปใบเดิมซ้ำ */
  slipRef?: string | null;
  /**
   *  none      = ไม่มี QR บนรูป (อาจเป็นภาพแคปหน้าจอ) ต้องตรวจด้วยตา
   *  unique    = QR อ่านได้และยังไม่เคยถูกใช้ แต่ยังไม่ได้ยืนยันกับธนาคาร
   *  verified  = บริการตรวจสลิปยืนยันแล้วว่าเงินเข้าจริงและยอดตรง
   *  mismatch  = ยืนยันแล้วแต่ยอดหรือปลายทางไม่ตรง
   *  failed    = เรียกบริการตรวจไม่สำเร็จ ให้ตรวจด้วยตา
   */
  slipCheck?: "none" | "unique" | "verified" | "mismatch" | "failed";
  /** ยอดที่บริการตรวจสลิปอ่านได้จริง ไว้เทียบกับที่ผู้โอนกรอก */
  slipAmount?: number | null;
  /** เวลาโอนตามสลิป */
  slipAt?: string | null;
  /** ธนาคารต้นทางตามสลิป */
  slipBank?: string | null;
  /** อนุมัติโดยระบบ ไม่ใช่คนกด */
